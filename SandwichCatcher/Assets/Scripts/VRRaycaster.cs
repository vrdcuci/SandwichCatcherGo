...
public class VRRaycaster : MonoBehaviour
{
    ...
    Transform Pointer
    {
        get
        {
            ...
            // This looks super awkward! Should probably fall back to a simple reticle!
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(laserPointer, out hit, maxRayDistance, ~excludeLayers))
        {
            ...
            if (hit.transform.gameObject.CompareTag("TeleportPad"))
            {
                if (OVRInput.GetUp(OVRInput.Button.PrimaryIndexTrigger))
                {
                    //player.transform.Translate(Vector3.forward);
                    Vector3 newPosition = new Vector3(hit.transform.position.x, 1.65f, hit.transform.position.z);
